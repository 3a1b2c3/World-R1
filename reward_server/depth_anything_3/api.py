...
from reward_server.depth_anything_3.cfg import create_object, load_config
from reward_server.depth_anything_3.registry import MODEL_REGISTRY
from reward_server.depth_anything_3.specs import Prediction
from reward_server.depth_anything_3.utils.export import export
from reward_server.depth_anything_3.utils.geometry import affine_inverse
from reward_server.depth_anything_3.utils.io.input_processor import InputProcessor
from reward_server.depth_anything_3.utils.io.output_processor import OutputProcessor
from reward_server.depth_anything_3.utils.logger import logger
...
def _normalize_model_name(model_name: str) -> str:
    ...
    if normalized in MODEL_REGISTRY:
        return normalized
    if normalized.startswith("da3nested-"):
        normalized = "reconstruction-nested-" + normalized[len("da3nested-"):]
    elif normalized.startswith("da3metric-"):
        normalized = "reconstruction-metric-" + normalized[len("da3metric-"):]
    elif normalized.startswith("da3mono-"):
        normalized = "reconstruction-mono-" + normalized[len("da3mono-"):]
    elif normalized.startswith("da3-"):
        normalized = "reconstruction-" + normalized[len("da3-"):]
    return normalized
...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(self, model_name: str = "reconstruction-large", **kwargs):
        """
        Initialize DepthAnything3 with specified preset.

        Args:
        model_name: The name of the model preset to use.
                    Examples: 'reconstruction-giant', 'reconstruction-large', 'reconstruction-metric-large', 'reconstruction-nested-giant-large'.
        **kwargs: Additional keyword arguments (currently unused).
        ...
        if self.model_name not in MODEL_REGISTRY:
            raise KeyError(
                f"Unknown reconstruction model '{model_name}'. "
    ...
    def forward(
        ...
        if device_type == "cuda":
            ...
            autocast_context = torch.autocast(device_type=device_type, dtype=torch.bfloat16)
        else:
